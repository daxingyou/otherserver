...
bool OperateFactory::initlize(const PropertySet& propSet)
{
	...
	printf("OperateFactory::initlize---start-");
	loadCheckInData();
	printf("OperateFactory::loadCheckInData---end-");
	loadGrowUpPlanData();
	printf("OperateFactory::loadGrowUpPlanData---end-");
	loadAccumulatePayMentData();
	printf("OperateFactory::loadAccumulatePayMentData---end-");
	loadPhoneMap();
	printf("OperateFactory::loadPhoneMap---end-");
	loadAccumulateLoginInData();
	printf("OperateFactory::loadAccumulateLoginInData---end-");
	loadDayPaymentData();
	printf("OperateFactory::loadDayPaymentData---end-");
	...
	printf("OperateFactory::initlize---end-");
	return true;
}
...
void OperateFactory::loadAccumulateLoginInData()
{
	...
	printf("OperateFactory::loadAccumulateLoginInData,iRecord=%d\n",iRecord);
	for(int i = 0; i < iRecord; ++i)
	{
		int iId = pTable->getInt(i, "累积登录天数");
		int iDropId = pTable->getInt(i, "奖励DropId");
		printf("OperateFactory::loadAccumulateLoginInData,iId=%d,iDropId=%d\n",iId,iDropId);
