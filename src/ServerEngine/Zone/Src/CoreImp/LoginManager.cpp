...
{
	printf("LoginManager::initlize------start---------");
	ITimerComponent* pTimeAxis = getComponent<ITimerComponent>(COMPNAME_TimeAxis, IID_ITimerComponent);
	assert(pTimeAxis);
	...
	printf("LoginManager::initlize------end---------");
...
{
	printf("LoginManager::onMessage------start---------");
	...
	switch(dwMsgID)
	{
		case GSProto::CMD_ALOGIN:
			printf("LoginManager::onMessage------GSProto::CMD_ALOGIN---------");
			onReqALoginMsg(pTmpQxMsg, msg);
			break;

		case GSProto::CMD_GETROLE:
			printf("LoginManager::onMessage------GSProto::CMD_GETROLE---------");
			onReqGetRole(pTmpQxMsg, msg);
			break;

		case GSProto::CMD_NEW_ROLE:
			printf("LoginManager::onMessage------GSProto::CMD_NEW_ROLE---------");
			onReqNewRole(pTmpQxMsg, msg);
			break;

		case GSProto::CMD_CHECK_ACTORNAME:
			printf("LoginManager::onMessage------GSProto::CMD_CHECK_ACTORNAME---------");
			OnReqcheckNameCanUse(pTmpQxMsg, msg);
			break;

		case GSProto::CMD_QUERY_FIRSTFIGHT:
			printf("LoginManager::onMessage------GSProto::CMD_QUERY_FIRSTFIGHT---------");
			onReqQueryFirstFight(pTmpQxMsg, msg);
			break;

		case GSProto::CMD_REQUEST_REFRESHGOLD:
			printf("LoginManager::onMessage------GSProto::CMD_REQUEST_REFRESHGOLD---------");
...
{
	printf("LoginManager::sendBattleInfo--------%s-----",pUserInfo->strUserId.c_str());
	...
	printf("LoginManager::sendBattleInfo------end---------");
}
...
void LoginManager::sendBattleMemMsg(const UserInfo* pUserInfo, const GSProto::FightDataAll& allData)
{
	...
	printf("LoginManager::sendBattleMemMsg,%s----",pUserInfo->strRsAddress.c_str());
...
{
	printf("LoginManager::sendBattleAction--------------%s-----",pUserInfo->strUserId.c_str());
	IMessageLayer* pMsgLayer = getComponent<IMessageLayer>(COMPNAME_MessageLayer, IID_IMessageLayer);
	assert(pMsgLayer);
...
{
	printf("LoginManager::sendBattleFin--------------%s-----",pUserInfo->strUserId.c_str());
	GSProto::SCMessage pkg;
...
{
	printf("LoginManager::onReqQueryFirstFight-----------------");
	const UserInfo* pUserInfo = getUserInfo(pQxMsg->strAccount);
	
	if(!pUserInfo) return;
	printf("LoginManager::onReqQueryFirstFight--------------%s-----",pUserInfo->strUserId.c_str());
	...
	printf("LoginManager::onReqQueryFirstFight--------end---------");
}

void LoginManager::generateFirstFight()
{
	printf("LoginManager::generateFirstFight-----------------");
	IGlobalCfg* pGlobalCfg = getComponent<IGlobalCfg>(COMPNAME_GlobalCfg, IID_IGlobalCfg);
	assert(pGlobalCfg);
	
	// 创建机器人
	string strHeroList = pGlobalCfg->getString("首场战斗机器人", "1006#0#1#0#1;1014#1#1#0#1;1031#2#1#0#1");
	printf("strHeroList:%s--", strHeroList.c_str());
	SvrRunLog("strHeroList:%s", strHeroList.c_str());
	...
	printf("LoginManager::generateFirstFight---------创建机器人  end--------");
	// 怪物组
	...
	printf("LoginManager::generateFirstFight--------all init end--------");
}


void LoginManager::fixFirstFightDlg(GSProto::FightDataAll& fightDataAll)
{
	printf("LoginManager::fixFirstFightDlg---------------");
	IGlobalCfg* pGlobalCfg = getComponent<IGlobalCfg>(COMPNAME_GlobalCfg, IID_IGlobalCfg);
	assert(pGlobalCfg);
...
void CheckNameCallback::callback_getNameDesc(taf::Int32 ret,  const ServerEngine::NameDesc& descInfo)
{
	...
	printf("LoginManager::callback_getNameDesc---------end------");
}
...
void LoginManager::processGetRole(const ServerEngine::PIRole& roleInfo, ServerEngine::VerifyKey& verifyKey)
{
	...
	printf("LoginManager::processGetRole------3----getComponent<IJZEntityFactory>----------\n");
	// 填充内容RS等
	...
	printf("LoginManager::processGetRole----------8---------\n");
	// 统计最高在线
	...
	printf("LoginManager::processGetRole----------9---------\n");
	...
	{
		printf("LoginManager::processGetRole---------in to -2144---------");
	}
	...
	printf("LoginManager::processGetRole----------10---------");
	//查询返还元宝
...
{
	const UserInfo* pUserInfo = getUserInfo(pQxMsg->strAccount);
	printf("onReqGetRole-------1");
	if(!pUserInfo) return;
	printf("onReqGetRole--------2");
	if(en_UserState_Running == pUserInfo->nState)
	{
		printf("onReqGetRole----processOnlineRoleGet---1");
...
{
	printf("LoginManager::onReqNewRole-------------1----------\n");
	const UserInfo* pUserInfo = getUserInfo(pQxMsg->strAccount);
	if(!pUserInfo) return;
	printf("LoginManager::onReqNewRole-------------2----------\n");
	if(en_UserState_NewRole != pUserInfo->nState)
	{
		printf("LoginManager::onReqNewRole------%d----------\n",pUserInfo->nState);
		return;
	}
	printf("LoginManager::onReqNewRole-------------3----------\n");
	...
	printf("LoginManager::onReqNewRole-------------4----------\n");
	if(!checkNameValid(pQxMsg->strAccount, tmpNewRole.strrolename() ) )
	{
		return;
	}
	printf("LoginManager::onReqNewRole-------------5----------\n");
	// 填充新角色数据
	IGlobalCfg* pGlobalCfg = getComponent<IGlobalCfg>(COMPNAME_GlobalCfg, IID_IGlobalCfg);
	assert(pGlobalCfg);
	printf("LoginManager::onReqNewRole-------------6----------");
	...
	printf("LoginManager::onReqNewRole-------------7----------");
	roleData.roleData = ServerEngine::JceToStr(tmpSaveData);
	printf("LoginManager::onReqNewRole-------------8----------");
	// 发送创建请求
	IMessageLayer* pMsgLayer = getComponent<IMessageLayer>(COMPNAME_MessageLayer, IID_IMessageLayer);
	assert(pMsgLayer);
	printf("LoginManager::onReqNewRole-------------9----------");
