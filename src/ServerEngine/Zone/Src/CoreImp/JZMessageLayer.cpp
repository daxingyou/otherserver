...
bool JZMessageLayer::initlize(const PropertySet& propSet)
{
	...
	printf("JZMessageLayer::initlize--------start-------");
	MessageLayer::initlize(propSet);

	printf("JZMessageLayer::initlize--------111-------\n");
	...
	printf("JZMessageLayer::initlize--------666-------\n");
	Application::getCommunicator()->stringToProxy(strLJSDKObj, m_LJSDKPrx);

	printf("JZMessageLayer::initlize------end---------");
	return true;
}
