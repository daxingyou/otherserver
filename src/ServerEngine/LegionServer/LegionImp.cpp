...
{	
	...
	printf("LegionImp::initialize()------1--redis,ip:%s,port:%d\n",strRedisIp.c_str(),iRedisPort);
	if(!m_redisClient.initlize(strRedisIp, iRedisPort, 1500) )
	{
		assert(false);
		return;
	}
	printf("LegionImp::initialize()----2----initRedis ok-------\n");
	...
	printf("LegionImp::initialize()----3----/LegionServer<NameObject> ok-------\n");
...
{
	printf("LegionImp::getLegion------start--key:%s---",strKey.c_str());
	...
	if(en_RedisRet_NoData == iRedisRet)
	{
		printf("LegionImp::getLegion------en_DataRet_NotExist-----");
		return ServerEngine::en_DataRet_NotExist;
	}
	else if(en_RedisRet_SysError == iRedisRet)
	{
		printf("LegionImp::getLegion------en_DataRet_OK1-----");
		return ServerEngine::en_DataRet_OK; 
	}
	else if(en_RedisRet_OK == iRedisRet)
	{
		strValue = TC_Base64::decode(strTmpValue);
		printf("LegionImp::getLegion------en_RedisRet_OK:%s-----",strValue.c_str());
		return ServerEngine::en_DataRet_OK;
	}
	printf("LegionImp::getLegion------en_DataRet_SysError-----");
	return ServerEngine::en_DataRet_SysError;
}
