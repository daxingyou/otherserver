...
void TC_EpollServer::Handle::handleImp()
{
    ...
    while (!getEpollServer()->isTerminate())
    {
        ...
        for (map<string, BindAdapterPtr>::iterator it = adapters.begin(); it != adapters.end(); ++it)
        {
            ...
            try
            {
                string tname = adapter->getName();
                // printf("TC_EpollServer::Handle::handleImp()-----adapterName:%s------",tname.c_str());
                while (adapter->waitForRecvQueue(recv, 0))
                {
                    ...
                    if (stRecvData.isOverload)
                    {
                        ...
                        // printf("TC_EpollServer::Handle::handleImp()-isOverload----adapterName:%s------",tname.c_str());
                    }                    
                    //关闭连接的通知消息
                    else if (stRecvData.isClosed)
                    {
                        handleClose(stRecvData);
                        // printf("TC_EpollServer::Handle::handleImp()-isClosed----adapterName:%s------",tname.c_str());
                    }
                    //数据在队列中已经超时了
                    else if ( (now - stRecvData.recvTimeStamp)*1000 > adapter->getQueueTimeout())
                    {
                        handleTimeout(stRecvData);
                        // printf("TC_EpollServer::Handle::handleImp()-isTimeout----adapterName:%s------",tname.c_str());
                    }                  
                    else
                    {
                        handle(stRecvData);
            ...
            catch (exception &ex)
            {
                ...
                string tname = adapter->getName();
                getEpollServer()->error("[Handle::handleImp] error:" + string(ex.what()) + "name:" + tname);
            }
            catch (...)
            {
                if(recv) 
...
int TC_EpollServer::Connection::parseProtocol(recv_queue::queue_type &o)
{
    try
    {
        while (true)
        {
            ...
            {
                //包不完全
                _pBindAdapter->getEpollServer()->error("recv data packet less");
                break;
            }
...
bool TC_EpollServer::accept(int fd)
{
    ...
    if (iRetCode > 0)
    {
        ...
        debug("accept [" + ip + ":" + TC_Common::tostr(port) + "][" + TC_Common::tostr(cs.getfd()) + "] debug 1");
    ...
    else
    {
        ...
        if(errno == EAGAIN)
        {
            debug("accept:[" + TC_Common::tostr(cs.getfd()) + "] debug 2");
            return false;
        }
        
        debug("accept:[" + TC_Common::tostr(cs.getfd()) + "] debug 3");
        return true;
    }
    debug("accept:[" + TC_Common::tostr(cs.getfd()) + "] debug 4");
    return true;
}
