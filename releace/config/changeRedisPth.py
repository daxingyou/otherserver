...
def changeOneLine(runpth,setpth):
    #pidfile /home/mage/Desktop/ServerEngine/redis/redis-fightdata0/redis-server.pid
    #/home/mage/Desktop/servercode/releace/ServerEngine
    outpth = runpth + setpth.split('ServerEngine')[1] + '\n'
    return outpth

def changelines(lines,cpth):

    runpth = GetParentPath(cpth) + '/ServerEngine'
    outstr = ''
    for l in lines:

        if l[0] != '#' and l.find('ServerEngine/redis/') != -1:
            if l[:7] == 'pidfile' or l[:7] == 'logfile' or l[:3] == 'dir':
                tmpline = ' '.join(l.split())
                tmpls = tmpline.split(' ')
                l = tmpls[0] + ' ' + changeOneLine(runpth,tmpls[1])
        outstr += l
    return outstr


def changeconfig(cpth,fpth):
    f = open(fpth,'r')
    lines = f.readlines()
    f.close()

    newfiletxt = changelines(lines,cpth)
    ...
    if os.path.exists(fpth + '_test'):
        os.remove(fpth + '_test')


def changeRedisConfPth(cpth):
    bpth = cpth + '/ServerEngine/redis'
    confphts = getAllFiles(bpth,fromatx = '.conf')
    for f in confphts:
        pthtmp = bpth + f
        # print(f)
        changeconfig(cpth,pthtmp)

def main():
    curpth = cur_file_dir()
    changeRedisConfPth(curpth)
